...
static int update_frame_pool(AVCodecContext* avctx, AVFrame* frame)
{
	...
	{
		if (avctx->codec_type == AVMEDIA_TYPE_VIDEO && pool->width == frame->width && pool->height == frame->height)
			return 0;
		...
		case AVMEDIA_TYPE_VIDEO:
		{
			...
			for (i = 0; i < 4; i++)
			{
				...
				if (size[i])
				{
					...
					pool->pools[i] = av_buffer_pool_init(size[i] + 16 + STRIDE_ALIGN - 1, av_buffer_allocz);
					if (!pool->pools[i])
					{
						ret = AVERROR(ENOMEM);
						goto fail;
