...
static int init_bundles(BinkContext* c)
{
	int bw, bh, blocks;
	uint8_t* tmp;
	int i;
...
static void free_bundles(BinkContext* c)
{
...
static int read_tree(GetBitContext* gb, Tree* tree)
{
	...
	if (get_bits1(gb))
	{
		...
		for (i = 0; i <= len; i++)
		{
			tree->syms[i] = (uint8_t)(get_bits(gb, 4));
			tmp1[tree->syms[i]] = 1;
		}
		for (i = 0; i < 16 && len < 16 - 1; i++)
			if (!tmp1[i])
				tree->syms[++len] = (uint8_t)(i);
	}
	else
	{
		len = get_bits(gb, 2);
...
static int binkb_read_bundle(BinkContext* c, GetBitContext* gb, int bundle_num)
{
	...
	if (bits <= 8)
	{
		...
		{
			for (i = 0; i < len; i++)
				*b->cur_dec++ = (uint8_t)(get_bits(gb, bits));
		}
		else
		{
			for (i = 0; i < len; i++)
				*b->cur_dec++ = (uint8_t)(get_bits(gb, bits) - mask);
		}
	}
	else
	{
		...
		{
			for (i = 0; i < len; i++)
				*dst++ = (int16_t)(get_bits(gb, bits));
		}
		else
		{
			for (i = 0; i < len; i++)
				*dst++ = (int16_t)(get_bits(gb, bits) - mask);
		}
		b->cur_dec = (uint8_t*)dst;
	}
	return 0;
			...
			switch (blk)
			{
				...
				case 1:
					...
					do
					{
						...
						if (mode)
						{
							...
							for (j = 0; j < run; j++)
								dst[coordmap[*scan++]] = (uint8_t)(v);
						}
						else
						{
							for (j = 0; j < run; j++)
								dst[coordmap[*scan++]] = (uint8_t)(binkb_get_value(c, BINKB_SRC_COLORS));
						}
					} while (i < 63);
					if (i == 63)
						dst[coordmap[*scan++]] = (uint8_t)(binkb_get_value(c, BINKB_SRC_COLORS));
				...
				case 2:
					...
					if ((quant_idx = read_dct_coeffs(c, gb, dctblock, bink_scan, &coef_count, coef_idx, qp)) < 0)
						return quant_idx;
					unquantize_dct_coeffs(dctblock, (const uint32_t*)binkb_intra_quant[quant_idx], coef_count, coef_idx, bink_scan);
				...
				case 4:
					...
					if ((quant_idx = read_dct_coeffs(c, gb, dctblock, bink_scan, &coef_count, coef_idx, qp)) < 0)
						return quant_idx;
					unquantize_dct_coeffs(dctblock, (const uint32_t*)binkb_inter_quant[quant_idx], coef_count, coef_idx, bink_scan);
					c->binkdsp.idct_add(dst, stride, dctblock);
					break;
				case 5:
					v = binkb_get_value(c, BINKB_SRC_COLORS);
					c->bdsp.fill_block_tab[1](dst, (uint8_t)(v), stride, 8);
					break;
				case 6:
					for (i = 0; i < 2; i++)
					...
					for (i = 0; i < 8; i++)
					{
						...
						for (j = 0; j < 8; j++, v >>= 1)
							dst[i * stride + j] = (uint8_t)(col[v & 1]);
					}
					break;
				case 7:
					xoff = binkb_get_value(c, BINKB_SRC_X_OFF);
			...
			switch (blk)
			{
					...
					switch (blk)
					{
						case RUN_BLOCK:
							...
							do
							{
								...
								if (get_bits1(gb))
								{
									...
									for (j = 0; j < run; j++)
										ublock[*scan++] = (uint8_t)(v);
								}
								else
								{
									for (j = 0; j < run; j++)
										ublock[*scan++] = (uint8_t)(get_value(c, BINK_SRC_COLORS));
								}
							} while (i < 63);
							if (i == 63)
								ublock[*scan++] = (uint8_t)(get_value(c, BINK_SRC_COLORS));
						...
						case INTRA_BLOCK:
							...
							if ((quant_idx = read_dct_coeffs(c, gb, dctblock, bink_scan, &coef_count, coef_idx, -1)) < 0)
								return quant_idx;
							unquantize_dct_coeffs(dctblock, (const uint32_t*)bink_intra_quant[quant_idx],
								coef_count, coef_idx, bink_scan);
							c->binkdsp.idct_put(ublock, 8, dctblock);
							break;
						case FILL_BLOCK:
							v = get_value(c, BINK_SRC_COLORS);
							c->bdsp.fill_block_tab[0](dst, (uint8_t)(v), stride, 16);
							break;
						case PATTERN_BLOCK:
							for (i = 0; i < 2; i++)
							...
							for (j = 0; j < 8; j++)
							{
								...
								for (i = 0; i < 8; i++, v >>= 1)
									ublock[i + j * 8] = (uint8_t)(col[v & 1]);
							}
							break;
						case RAW_BLOCK:
							for (j = 0; j < 8; j++)
								for (i = 0; i < 8; i++)
									ublock[i + j * 8] = (uint8_t)(get_value(c, BINK_SRC_COLORS));
							break;
						default:
							return AVERROR_INVALIDDATA;
					}
				...
				case RUN_BLOCK:
					...
					do
					{
						...
						{
							for (j = 0; j < run; j++)
								dst[coordmap[*scan++]] = (uint8_t)(get_value(c, BINK_SRC_COLORS));
						}
					} while (i < 63);
					if (i == 63)
						dst[coordmap[*scan++]] = (uint8_t)(get_value(c, BINK_SRC_COLORS));
				...
				case INTRA_BLOCK:
					...
					if ((quant_idx = read_dct_coeffs(c, gb, dctblock, bink_scan, &coef_count, coef_idx, -1)) < 0)
						return quant_idx;
					unquantize_dct_coeffs(dctblock, (const uint32_t*)bink_intra_quant[quant_idx], coef_count, coef_idx, bink_scan);
					c->binkdsp.idct_put(dst, stride, dctblock);
					break;
				case FILL_BLOCK:
					v = get_value(c, BINK_SRC_COLORS);
				...
				case INTER_BLOCK:
					...
					if ((quant_idx = read_dct_coeffs(c, gb, dctblock, bink_scan, &coef_count, coef_idx, -1)) < 0)
						return quant_idx;
					unquantize_dct_coeffs(dctblock, (const uint32_t*)bink_inter_quant[quant_idx], coef_count, coef_idx, bink_scan);
					c->binkdsp.idct_add(dst, stride, dctblock);
					break;
				case PATTERN_BLOCK:
					for (i = 0; i < 2; i++)
...
static int decode_frame(AVCodecContext* avctx, AVFrame* frame, int* got_frame, AVPacket* pkt)
{
	...
	for (plane = 0; plane < 3; plane++)
	{
		...
		{
			if ((ret = binkb_decode_plane(c, frame, &gb, plane_idx, c->frame_num == 1, !!plane)) < 0)
				return ret;
		}
...
static void bink_init_vlcs(void)
{
...
static void binkb_calc_quant(void)
{
...
#define C (1LL<<30)
	for (i = 0; i < 64; i++)
		inv_bink_scan[bink_scan[i]] = (uint8_t)(i);
	...
	for (j = 0; j < 16; j++)
	{
		for (i = 0; i < 64; i++)
		{
			...
			binkb_intra_quant[j][k] = binkb_intra_seed[i] * (int64_t)s[i] * binkb_num[j] / (binkb_den[j] * (C >> 12));
			binkb_inter_quant[j][k] = binkb_inter_seed[i] * (int64_t)s[i] * binkb_num[j] / (binkb_den[j] * (C >> 12));
		}
	}
}
