...
static int decode_init(AVCodecContext* avctx)
{
	...
	if (avctx->codec->id == AV_CODEC_ID_BINKAUDIO_RDFT)
		s->root = (float)(2.0 / (sqrt(s->frame_len) * 32768.0));
	else
		s->root = (float)(s->frame_len / (sqrt(s->frame_len) * 32768.0));
	for (i = 0; i < 96; i++)
	{
	...
	{
		float scale = (float)(1.0 / (1 << frame_len_bits));
...
static int decode_block(BinkAudioContext* s, float** out, int use_dct, int channels, int ch_offset)
{
	...
	for (ch = 0; ch < channels; ch++)
	{
		...
		while (i < s->frame_len)
		{
			...
			if (width == 0)
			{
				...
				i = j;
				while (s->bands[k] < (unsigned int)i)
					q = quant[k++];
			}
			else
			{
		...
		{
			for (int i2 = 2; i2 < s->frame_len; i2 += 2)
				coeffs[i2 + 1] *= -1;
	...
	for (ch = 0; ch < channels; ch++)
	{
		...
		{
			j2 = ch;
			for (i = 0; i < s->overlap_len; i++, j2 += channels)
				out[ch + ch_offset][i] = (s->previous[ch + ch_offset][i] * (count - j2) +
					out[ch + ch_offset][i] * j2) / count;
		}
...
static void decode_flush(AVCodecContext* avctx)
{
	...
	/* s->pkt coincides with avctx->internal->in_pkt and is unreferenced generically when flushing. */
	s->first = 1;
	s->ch_offset = 0;
}
