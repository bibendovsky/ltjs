...
static void add_pixels8_c(uint8_t* pixels, int16_t* block, int line_size)
{
	...
	for (i = 0; i < 8; i++)
	{
		pixels[0] = (uint8_t)(pixels[0] + block[0]);
		pixels[1] = (uint8_t)(pixels[1] + block[1]);
		pixels[2] = (uint8_t)(pixels[2] + block[2]);
		pixels[3] = (uint8_t)(pixels[3] + block[3]);
		pixels[4] = (uint8_t)(pixels[4] + block[4]);
		pixels[5] = (uint8_t)(pixels[5] + block[5]);
		pixels[6] = (uint8_t)(pixels[6] + block[6]);
		pixels[7] = (uint8_t)(pixels[7] + block[7]);
		pixels += line_size;
		block += 8;
	}
}
