...
#define SR_TABLE(len)                                              \
static void TX_TAB(ff_tx_init_tab_ ##len)(void)            \
{                                                                  \
    double freq = 2*M_PI/len;                                      \
    TXSample *tab = TX_TAB(ff_tx_tab_ ##len);                      \
                                                                   \
    for (int i = 0; i < len/4; i++)                                \
        *tab++ = (TXSample)RESCALE(cos(i*freq));                   \
                                                                   \
    *tab = 0;                                                      \
}
SR_POW2_TABLES
...
static int sr_tabs_init_once[] = {
#define SR_TABLE(len) 0,
    SR_POW2_TABLES
#undef SR_TABLE
};
...
static void TX_TAB(ff_tx_init_tab_53)(void)
{
    ...
    TX_TAB(ff_tx_tab_53)[0] = (TXSample)RESCALE(cos(2 * M_PI /  5));
    TX_TAB(ff_tx_tab_53)[1] = (TXSample)RESCALE(cos(2 * M_PI /  5));
    TX_TAB(ff_tx_tab_53)[2] = (TXSample)RESCALE(cos(2 * M_PI / 10));
    TX_TAB(ff_tx_tab_53)[3] = (TXSample)RESCALE(cos(2 * M_PI / 10));
    TX_TAB(ff_tx_tab_53)[4] = (TXSample)RESCALE(sin(2 * M_PI /  5));
    TX_TAB(ff_tx_tab_53)[5] = (TXSample)RESCALE(sin(2 * M_PI /  5));
    TX_TAB(ff_tx_tab_53)[6] = (TXSample)RESCALE(sin(2 * M_PI / 10));
    TX_TAB(ff_tx_tab_53)[7] = (TXSample)RESCALE(sin(2 * M_PI / 10));

    /* 3pt */
    TX_TAB(ff_tx_tab_53)[ 8] = (TXSample)RESCALE(cos(2 * M_PI / 12));
    TX_TAB(ff_tx_tab_53)[ 9] = (TXSample)RESCALE(cos(2 * M_PI / 12));
    TX_TAB(ff_tx_tab_53)[10] = (TXSample)RESCALE(cos(2 * M_PI /  6));
    TX_TAB(ff_tx_tab_53)[11] = (TXSample)RESCALE(cos(8 * M_PI /  6));
}

static void TX_TAB(ff_tx_init_tab_7)(void)
{
    TX_TAB(ff_tx_tab_7)[0] = (TXSample)RESCALE(cos(2 * M_PI /  7));
    TX_TAB(ff_tx_tab_7)[1] = (TXSample)RESCALE(sin(2 * M_PI /  7));
    TX_TAB(ff_tx_tab_7)[2] = (TXSample)RESCALE(sin(2 * M_PI / 28));
    TX_TAB(ff_tx_tab_7)[3] = (TXSample)RESCALE(cos(2 * M_PI / 28));
    TX_TAB(ff_tx_tab_7)[4] = (TXSample)RESCALE(cos(2 * M_PI / 14));
    TX_TAB(ff_tx_tab_7)[5] = (TXSample)RESCALE(sin(2 * M_PI / 14));
}

static void TX_TAB(ff_tx_init_tab_9)(void)
{
    TX_TAB(ff_tx_tab_9)[0] = (TXSample)RESCALE(cos(2 * M_PI /  3));
    TX_TAB(ff_tx_tab_9)[1] = (TXSample)RESCALE(sin(2 * M_PI /  3));
    TX_TAB(ff_tx_tab_9)[2] = (TXSample)RESCALE(cos(2 * M_PI /  9));
    TX_TAB(ff_tx_tab_9)[3] = (TXSample)RESCALE(sin(2 * M_PI /  9));
    TX_TAB(ff_tx_tab_9)[4] = (TXSample)RESCALE(cos(2 * M_PI / 36));
    TX_TAB(ff_tx_tab_9)[5] = (TXSample)RESCALE(sin(2 * M_PI / 36));
...
static int nptwo_tabs_init_once[] = {0, 0, 0};

void TX_TAB(ff_tx_init_tabs)(int len)
{
    ...
    if (factor_2) {
        ...
        for (int i = 0; i <= idx; i++)
        {
            if (!sr_tabs_init_once[i])
            {
                sr_tabs_init_funcs[i]();
                sr_tabs_init_once[i] = 1;
            }
        }
        len >>= factor_2;
    }

    for (size_t i = 0; i < FF_ARRAY_ELEMS(nptwo_tabs_init_data); i++) {
        int f, f_idx = 0;
        ...
        while ((f = nptwo_tabs_init_data[i].factors[f_idx++])) {
            ...
            if (!nptwo_tabs_init_once[i])
            {
                nptwo_tabs_init_data[i].func();
                nptwo_tabs_init_once[i] = 1;
            }
...
static inline void fft3(TXComplex *out, TXComplex *in,
                                  ptrdiff_t stride)
{
    TXComplex tmp[3];
...
static inline void NAME(TXComplex *out, TXComplex *in,    \
                                  ptrdiff_t stride)                 \
{                                                                   \
...
static inline void fft7(TXComplex *out, TXComplex *in,
                                  ptrdiff_t stride)
{
...
static inline void fft9(TXComplex *out, TXComplex *in,
                                  ptrdiff_t stride)
{
...
static inline void fft15(TXComplex *out, TXComplex *in,
                                   ptrdiff_t stride)
{
    TXComplex tmp[15];
...
static int TX_NAME(ff_tx_fft_factor_init)(AVTXContext *s,
...
static int TX_NAME(ff_tx_fft_sr_codelet_init)(AVTXContext *s,
                                                      const FFTXCodelet *cd,
                                                      uint64_t flags,
                                                      FFTXCodeletOptions *opts,
                                                      int len, int inv,
...
static int TX_NAME(ff_tx_fft_init)(AVTXContext *s,
                                           const FFTXCodelet *cd,
                                           uint64_t flags,
                                           FFTXCodeletOptions *opts,
                                           int len, int inv,
...
static int TX_NAME(ff_tx_fft_inplace_small_init)(AVTXContext *s,
                                                         const FFTXCodelet *cd,
                                                         uint64_t flags,
                                                         FFTXCodeletOptions *opts,
                                                         int len, int inv,
...
static int TX_NAME(ff_tx_fft_init_naive_small)(AVTXContext *s,
                                                       const FFTXCodelet *cd,
                                                       uint64_t flags,
                                                       FFTXCodeletOptions *opts,
                                                       int len, int inv,
                                                       const void *scale)
{
    ...
    for (int i = 0; i < len; i++) {
        for (int j = 0; j < len; j++) {
            ...
            s->exp[i*j] = (TXComplex){
                (TXSample)RESCALE(cos(factor)),
                (TXSample)RESCALE(sin(factor)),
            };
        }
    }
...
static int TX_NAME(ff_tx_fft_pfa_init)(AVTXContext *s,
                                               const FFTXCodelet *cd,
                                               uint64_t flags,
                                               FFTXCodeletOptions *opts,
                                               int len, int inv,
                                               const void *scale)
{
    ...
    int ret, *tmp, ps = (int)(flags & FF_TX_PRESHUFFLE);
...
static int TX_NAME(ff_tx_mdct_naive_init)(AVTXContext *s,
                                                  const FFTXCodelet *cd,
                                                  uint64_t flags,
                                                  FFTXCodeletOptions *opts,
                                                  int len, int inv,
                                          ...
                                          void *_src, ptrdiff_t stride)
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        dst[i*stride] = (TXSample)RESCALE(sum*scale);
    }
}
                                          ...
                                          void *_src, ptrdiff_t stride)
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        dst[i +   0] = (TXSample)RESCALE( sum_d*scale);
        dst[i + len] = (TXSample)RESCALE(-sum_u*scale);
    }
}
...
static int TX_NAME(ff_tx_mdct_init)(AVTXContext *s,
                                            const FFTXCodelet *cd,
                                            uint64_t flags,
                                            FFTXCodeletOptions *opts,
                                            int len, int inv,
...
static int TX_NAME(ff_tx_mdct_inv_full_init)(AVTXContext *s,
                                                     const FFTXCodelet *cd,
                                                     uint64_t flags,
                                                     FFTXCodeletOptions *opts,
                                                     int len, int inv,
...
static int TX_NAME(ff_tx_mdct_pfa_init)(AVTXContext *s,
                                                const FFTXCodelet *cd,
                                                uint64_t flags,
                                                FFTXCodeletOptions *opts,
                                                int len, int inv,
...
static int TX_NAME(ff_tx_rdft_init)(AVTXContext *s,
                                            const FFTXCodelet *cd,
                                            uint64_t flags,
                                            FFTXCodeletOptions *opts,
                                            int len, int inv,
                                            const void *scale)
{
    ...
    *tab++ = (TXSample)RESCALE((inv ? 0.5 : 1.0) * m);
    *tab++ = (TXSample)RESCALE(inv ? 0.5*m : 1.0*m);
    *tab++ = (TXSample)RESCALE( m);
    *tab++ = (TXSample)RESCALE(-m);

    *tab++ = (TXSample)RESCALE( (0.5 - 0.0) * m);
    if (r2r)
        *tab++ = (TXSample)(1 / s->scale_f);
    else
        *tab++ = (TXSample)RESCALE( (0.0 - 0.5) * m);
    *tab++ = (TXSample)RESCALE( (0.5 - inv) * m);
    *tab++ = (TXSample)RESCALE(-(0.5 - inv) * m);

    for (int i = 0; i < len4; i++)
        *tab++ = (TXSample)RESCALE(cos(i*f));

    tab = ((TXSample *)s->exp) + len4 + 8;

    for (int i = 0; i < len4; i++)
        *tab++ = (TXSample)(RESCALE(cos(((len - i*4)/4.0)*f)) * (inv ? 1 : -1));
...
static int TX_NAME(ff_tx_dct_init)(AVTXContext *s,
                                           const FFTXCodelet *cd,
                                           uint64_t flags,
                                           FFTXCodeletOptions *opts,
                                           int len, int inv,
                                           const void *scale)
{
    ...
    for (int i = 0; i < len; i++)
        tab[i] = (TXSample)RESCALE(cos(i*freq)*(!inv + 1));

    if (inv) {
        for (int i = 0; i < len/2; i++)
            tab[len + i] = (TXSample)RESCALE(0.5 / sin((2*i + 1)*freq));
    } else {
        for (int i = 0; i < len/2; i++)
            tab[len + i] = (TXSample)RESCALE(cos((len - 2*i - 1)*freq));
    }
                                 ...
                                 void *_src, ptrdiff_t stride)
{
    ...
    for (int i = 0; i < len2; i++) {
        ...
        tmp1 = (TXSample)((in1 + in2)*0.5);
        tmp2 = (in1 - in2)*s;

        src[i]           = (TXSample)(tmp1 + tmp2);
        src[len - i - 1] = (TXSample)(tmp1 - tmp2);
    }
                                  ...
                                  void *_src, ptrdiff_t stride)
{
    ...
    for (int i = 0; i < len2; i++) {
        ...
        dst[i]            = (TXSample)(tmp1 + tmp2);
        dst[len - i - 1]  = (TXSample)(tmp1 - tmp2);
    }
}
...
static int TX_NAME(ff_tx_dcstI_init)(AVTXContext *s,
                                             const FFTXCodelet *cd,
                                             uint64_t flags,
                                             FFTXCodeletOptions *opts,
                                             int len, int inv,
...
int TX_TAB(ff_tx_mdct_gen_exp)(AVTXContext *s, int *pre_tab)
{
    ...
    for (int i = 0; i < len4; i++) {
        ...
        s->exp[off + i] = (TXComplex){ (TXSample)RESCALE(cos(alpha) * scale),
                                       (TXSample)RESCALE(sin(alpha) * scale) };
    }
