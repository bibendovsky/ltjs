...
LTRESULT CSoundBuffer::InitFromCompressed(CSoundBuffer &compressedSoundBuffer)
{
    ...
    if (compressedSoundBuffer.m_WaveHeader.m_WaveFormat.tag == ltjs::WaveFormatTag::ima_adpcm)
    {
    ...
    else if (compressedSoundBuffer.m_WaveHeader.m_WaveFormat.tag == ltjs::WaveFormatTag::mp3)
    {
...
inline void CSoundBuffer::CalcSampleType(S32 &sampleType, ltjs::WaveFormatEx &waveFormat)
{
    // Get the MSS sample type.
    switch(waveFormat.bit_depth * (waveFormat.channel_count + 1) >> 3)
    {
        case 4:
...
LTRESULT CSoundBuffer::LoadData()
{
    ...
    if (m_WaveHeader.m_WaveFormat.tag == ltjs::WaveFormatTag::ima_adpcm && 
        m_WaveHeader.m_WaveFormat.bit_depth != 4)
        bRet = LTFALSE;
    if (!bRet)
    {
        // Close the file
    ...
    m_dwDuration = (uint32)((1000.0f * (float)m_WaveHeader.m_dwDataSize / (float)m_WaveHeader.m_WaveFormat.avg_bytes_per_sec) + 0.5f);

    // Calculate the loop points in time.
    if (m_WaveHeader.m_CuePoint.m_bValid)
    {
        // Make sure it's got some valid data.
        if (m_WaveHeader.m_CuePoint.m_dwLength > 0 && m_WaveHeader.m_WaveFormat.sample_rate)
        {
            m_dwLoopPoints[0] = m_WaveHeader.m_CuePoint.m_dwPosition * 1000L / 
                m_WaveHeader.m_WaveFormat.sample_rate;
    ...
    if (!(GetSoundBufferFlags() & SOUNDBUFFERFLAG_STREAM))
    {
            ...
            GetSoundBufferFlags() & (SOUNDBUFFERFLAG_DECOMPRESSATSTART | SOUNDBUFFERFLAG_DECOMPRESSONLOAD))
        {
            ...
            m_SoundInfo.format = static_cast<std::int32_t>(m_WaveHeader.m_WaveFormat.tag);
            m_SoundInfo.data_len = m_WaveHeader.m_dwDataSize;
            m_SoundInfo.rate = m_WaveHeader.m_WaveFormat.sample_rate;
            m_SoundInfo.bits = m_WaveHeader.m_WaveFormat.bit_depth;
            m_SoundInfo.channels = m_WaveHeader.m_WaveFormat.channel_count;
            m_SoundInfo.samples = m_WaveHeader.m_dwSamples;
            m_SoundInfo.block_size = m_WaveHeader.m_WaveFormat.block_align;
            ...
            {
                if (GetWaveFormat().bit_depth == 16 && GetWaveFormat().tag == ltjs::WaveFormatTag::pcm)
                {
                    m_WaveHeader.m_dwDataSize /= 2;
                    GetWaveFormat().bit_depth = 8;
                    GetWaveFormat().avg_bytes_per_sec /= 2;
                    GetWaveFormat().block_align /= 2;
...
LTRESULT CSoundBuffer::LoadDataFromDecompressed()
{
    ...
    m_dwDuration = (uint32)((1000.0f * (float)m_WaveHeader.m_dwDataSize / 
        (float)m_WaveHeader.m_WaveFormat.avg_bytes_per_sec) + 0.5f);
