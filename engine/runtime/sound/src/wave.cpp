	...
	switch( dwType )
	{
		...
		case s_tagDATA:
		{
			...
			if( dwForm == s_tagWAVE )
			{
				...
				if( waveHeader.m_WaveFormat.tag == ltjs::WaveFormatTag::ima_adpcm )
				{
					dwSamplesPerBlock = 4 << ( waveHeader.m_WaveFormat.channel_count / 2 );
					dwSamplesPerBlock = 1 + ( waveHeader.m_WaveFormat.block_align - dwSamplesPerBlock ) * 8 / dwSamplesPerBlock;
					waveHeader.m_dwSamples = (( waveHeader.m_dwDataSize + waveHeader.m_WaveFormat.block_align - 1 ) / 
						waveHeader.m_WaveFormat.block_align ) * dwSamplesPerBlock;
				}
				else
				{
...
LTBOOL GetWaveInfo( ILTStream &inStream, CWaveHeader &waveHeader )
{
	...
	if( waveHeader.m_WaveFormat.tag == ltjs::WaveFormatTag::pcm ||
		waveHeader.m_WaveFormat.tag == ltjs::WaveFormatTag::ima_adpcm ||
		waveHeader.m_WaveFormat.tag == ltjs::WaveFormatTag::mp3 )
		return LTTRUE;
